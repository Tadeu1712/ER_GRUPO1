...
{
     echo "erro na variavel de sessão!!!!";
}
 else{
?>
...
if( $state==null ){
    ?>
        ...
        <div>
            <h2  style="margin-top: 5px; color: white; font-size:35px; text-align: center; text-shadow: 1px 2px grey; " >Proposta Pública</h2>
            </div>    
        <div style="border: 1px solid white;">
                <select name="area"  style="color:white; width:100%; background:transparent; border: 0px; font-size: 15px; height:30px;">
                    <option value='0'>Escolher a Área</option>
                    <option value="3">Canalizador</option>
            ...
            <p style="font-size: 15px; color: white">Morada: </p>
              ...
              <div>
                <input style=" border:1px solid white; margin-bottom: 15px;" type="date"  name="dataLimiteInput">
              </div>
            <div>
            <p style="font-size: 15px; color: white">Descrição: </p>
                <textarea style=" border:1px solid white; margin-bottom: 15px;" placeholder="Descrição" rows="4" cols="50" name="descricaoInput" form="form1"></textarea> 
            </div>
            <div>
    ...
    if(in_array($imageActualExt,$allowed)){
      if($imageError===0){
        ...
        doQuery("INSERT  INTO `pedido` (`area_id`,`morada`,`descricao`,`aceita_proposta`,`dataLimite`,`foto`,`pessoa_id`) 
    VALUES ('$area', '$morada', '$descricao',0,'$dataLim','$imageNewName',$id_cliente)");
      // 
        ?>
        ...
        echo "<div class='showData'> Área: ".$area."</div>";
        ...
        echo "<img src='../source.gif' heigth='100' widht='100' />";
